    ...
    *,
    ...
    try:
        ...
        if isinstance(schemas, dict):
            schema_key = getattr(builder, "schema", None)
            if schema_key is not None and schema_key in schemas:
                channels = schemas[schema_key]
            else:
                state_schema = getattr(builder, "state_schema", None)
                if state_schema is not None and state_schema in schemas:
                    channels = schemas[state_schema]
                elif len(schemas) == 1:
                    channels = next(iter(schemas.values()))
    except Exception:
        channels = None

    if isinstance(channels, dict) and "active_agent" not in channels:
